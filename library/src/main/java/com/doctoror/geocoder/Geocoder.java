...
public final class Geocoder {
    ...
    public List<Address> getFromLocation(final double latitude, final double longitude,
        ...
        if (data != null) {
            this.parseJson(results, maxResults, data, parseAddressComponents);
        }
        return results;
    }
    ...
    private void parseJson(@NonNull final List<Address> address,
                ...
                for (int i = 0; i < maxResults && i < results.length(); i++) {
                    ...
                    if (result.has(GEOMETRY)) {
                        ...
                        if (geometry.has(LOCATION)) {
                            ...
                            current.setLocationLat(location.getDouble("lat"));
                            current.setLocationLng(location.getDouble("lng"));
                        }
                    }
